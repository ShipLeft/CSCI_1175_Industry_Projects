...
/**
 * <h1>UnweightedGraph</h1>
 *
 * <p>This class has been modified to use getACycle to find any cycles inside of the given graph(s).</p>
 *
 * <p>Created: Unknown    Edited: 01/11/2022</p>
 *
...
public class UnweightedGraph<V> implements Graph<V> {
    ...
    public List<Integer> getACycle(){
        ...
        for(int i = 0; i < parent.length; i++)
            parent[i] = -1; //Initialize int array

        while(verts.size() > 0){ //While verts is not 0
             int v = verts.get(0);
                Stack<Integer> stack = new Stack<>(); //New stack instead of using recursion
                stack.push(v); //Push v to the new stack
                verts.remove(new Integer(v)); //Remove v from the list
                visited[v] = true; //Mark v as visited

                while(!stack.empty()){
                    int x = (int) stack.peek(); //Set x equal to the top of the stack
                    if(neighbors.get(x).size() == 0) {
                        ...
                        for(int i = 0; i <  neighbors.get(x).size(); i++){
                            Edge e = neighbors.get(x).get(i); //edge between x and neighbor
                            if (!visited[e.v]){
                                parent[e.v] = x; //Set parent equal to x
                                stack.push(e.v); //push e.v into the stack
                                visited[e.v] = true; //Mark visited e.v true
                                verts.remove(new Integer(e.v));
                                neighbors.get(x).remove(i); //Remove i from neighbors
                                break;
                            }
                            else if(e.v != parent[x]){
                                ArrayList<Integer> notParent = new ArrayList<>();
                                notParent.add(e.v);
                                while(x != e.v && x != -1){
                                    notParent.add(x);
                                    x = parent[x];
