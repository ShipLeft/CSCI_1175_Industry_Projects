 ...
 *
 * <p>This class has been modified to use the non-recursive DFS for Exercise28_03</p>
 *
 * <p>Created: Unknown</p>
 *
...
public class UnweightedGraph<V> implements Graph<V> {
    ...
    public SearchTree dfsNonRecurs(int v) {
        ...
        stack.push(v); //Push v into the top of the stack
        searchOrder.add(v); //Add v to the searchOrder
        isVisited[v] = true; //Mark v as visited
        while(!stack.empty()){ //While the stack is not empty
            int x = stack.peek(); //Call x for the top of the stack
            if(neighbors.get(x).size() == 0) {
                stack.pop(); //Pop the top of the stack if there aren't any vertices to add
                continue;
            }
            else {
                for(int i = neighbors.get(x).size() - 1; i >= 0; i--){
                    Edge e = neighbors.get(x).get(i); //e for the edge of x(i)
                    neighbors.get(x).remove(i); //Remove the ending vertex of x

                    if(!isVisited[e.v]){
                        parent[e.v] = x; //Parent of vertex is x
                        stack.push(e.v); //Push vertex to the top of the stack
                        isVisited[e.v] = true; //Mark the vertex as visited
                        searchOrder.add(e.v); //Add the vertex to the searchOrder
                        break;
                    }
                }
