 ...
 *
 * <p>This class has been modified to include the getPath method for finding the
 * shortest path from one city to another city using parents to find the route.</p>
 *
 * <p>Created: Unknown</p>
 *
...
public class UnweightedGraph<V> implements Graph<V> {
    ...
    public List<Integer> getPath(int u, int v) {
        SearchTree path = bfs(u); //Path of the bfs to search
        ArrayList<Integer> pathNodes = new ArrayList<>(); //List for the nodes to be returned

        while(v != -1){
            pathNodes.add(v); //Add the current value to the ArrayList
            v = path.parent[v]; //Set the current value equal to the parent of the value
        }
        Collections.reverse(pathNodes); //Reverse the order as to give the order correctly
        return pathNodes; //Return the path inside of the ArrayList
    }
